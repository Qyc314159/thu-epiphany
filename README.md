...
|---|-------------|---------|------------------------|
| ① | **APP_ID** | [飞书开放平台](https://open.feishu.cn/app) → 你的应用 →「凭证与基础信息」| `feishu.app_id` |
| ② | **APP_SECRET** | 同上页面 →「凭证与基础信息」点击显示 | `feishu.app_secret` |
| ③ | **doc_token** | 新建飞书文档 → 从 URL 中提取：`https://xxx.feishu.cn/docx/<这里>` | `doc_token` |
| ④ | **inspiration_pool_block_id** | 启动服务器后访问 `GET /api/scan-blocks` 自动获取 | `inspiration_pool_block_id` |
| ⑤ | **notify_user_open_id** | 向你的 Bot 发一条消息 → 查服务端日志获得 | `notify_user_open_id` |

> ⚠️ 创建 Bot 后记得在「权限管理」添加 `docx:document` 等权限，并**发布版本**才能生效。详细步骤见 [MANUAL.md → 第3节](MANUAL.md#3-飞书端配置)。
